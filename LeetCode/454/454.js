...
var fourSumCount3 = function(A, B, C, D) {
  ...
  A.forEach(a => {
    B.forEach(b => {
        a_b_hash[a + b]={ val: a + b, type: "a",num:a_b_hash[a + b]?a_b_hash[a + b].num+1:1}
    });
  });
  C.forEach(a => {
    D.forEach(b => {
        c_d_hash[a + b]={ val: a + b, type: "b",num:c_d_hash[a + b]?c_d_hash[a + b].num+1:1}
        });
  });
  for (let i in a_b_hash){a_b_c_d.push(a_b_hash[i])}
  for (let i in c_d_hash){a_b_c_d.push(c_d_hash[i])}
  const hashSet = {};
  for (let i in a_b_c_d) {
    const expect = -a_b_c_d[i].val;
    const type = `${expect}_${a_b_c_d[i].type === "a" ? "b" : "a"}`;
